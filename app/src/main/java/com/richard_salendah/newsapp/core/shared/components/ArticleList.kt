...
fun ArticlesListPreview() {
    AppTheme {
        ArticlesListOnline(articles = createMockLazyPagingItems(), onClick = {})
    }
}

@Composable
fun createMockLazyPagingItems(): LazyPagingItems<Article> {
        ...
        pagingSourceFactory = {
            object : PagingSource<Int, Article>() {
                override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Article> {
                    ...
                    totalNewsCount += mockArticles.size

                    return LoadResult.Page(
                        data = mockArticles,
                        prevKey = null,
                        nextKey = if (totalNewsCount == mockArticles.size) null else page + 1
                    )
                }
